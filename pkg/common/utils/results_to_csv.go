...
// WriteResultsCSV writes experiment results to a CSV file.
// The output directory is configurable via the outputDir parameter.
func WriteResultsCSV(instanceName string, rows []Row, outputDir string) error {
	...
	// Add optional fields if any row has them
	hasLNSIters := false
	for _, r := range rows {
		if r.AvgLNSIters > 0 {
			hasLNSIters = true
	...
	if hasLNSIters {
		header = append(header, "avg_lns_iterations")
	}
	...
	for _, r := range rows {
		...
		if hasLNSIters {
			rec = append(rec, fmt.Sprintf("%.1f", r.AvgLNSIters))
		}
