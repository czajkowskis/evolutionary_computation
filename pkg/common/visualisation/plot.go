...
// The outputDir parameter specifies where to save the plot file.
func PlotSolution(nodes []data.Node, path []int, title string, filename string,
	xMin, xMax, yMin, yMax float64, outputDir string) error {
	...
	xAxisPts := plotter.XYs{
		plotter.XY{X: xMin, Y: 0},
		plotter.XY{X: xMax, Y: 0},
	}
	yAxisPts := plotter.XYs{
		plotter.XY{X: 0, Y: yMin},
		plotter.XY{X: 0, Y: yMax},
	}
	...
	// --- X-axis arrow  ---
	ax1Pts := plotter.XYs{
		plotter.XY{X: xMax - arrowSize, Y: 0.25 * arrowSize},
		plotter.XY{X: xMax, Y: 0},
	}
	ax2Pts := plotter.XYs{
		plotter.XY{X: xMax - arrowSize, Y: -0.25 * arrowSize},
	...
	ay1Pts := plotter.XYs{
		plotter.XY{X: -0.25 * arrowSize * aspect, Y: yMax - arrowSize*lengthRatio},
		plotter.XY{X: 0, Y: yMax},
	}
	ay2Pts := plotter.XYs{
		plotter.XY{X: 0.25 * arrowSize * aspect, Y: yMax - arrowSize*lengthRatio},
	...
	filePath := filepath.Join(outputDir, fmt.Sprintf("%s.png", filename))
